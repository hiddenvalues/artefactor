...
Publishing: use create_artefact to publish, update_artefact to iterate in place (HTML is a full replacement, not a patch), set_visibility to share. Moving a public artefact to any other tier clears its link protection (password and expiry); link protection is set only in the Artefactor web app, never over the connector. Saved data is own-only by default: each viewer loads only their own (the owner sees everyone's), which suits surveys and forms; set_data_visibility with "shared" lets viewers load each other's. Read back with get_artefact_html (the stored HTML, to derive a new artefact or to re-read one before updating it) and get_artefact_data (your own saved blob — or, with author, another user's; see below — verbatim, plus the declared schema, dataAuthorCount, and the currentPayloadVersion / authoredAgainstVersion pin). Both refuse an over-cap result rather than truncating — the user can then get the file from the Artefactor web app's "Download HTML" menu item.
...
Each snapshot you read is ONE user's blob, not the population (on your own artefacts list_artefact_data_authors shows everyone and their pins): blobs may sit on older key versions, be partial, or have been written by HTML two revisions back, so a migration must tolerate shapes you never saw. Staleness per user: authoredAgainstVersion null = unknown, treat as possibly stale; ≠ currentPayloadVersion = written against older HTML, migration owed; = current = matches what is deployed. (That pin is backend-set and mechanical; the declared schema's own version is author-set and semantic — keep both.) Trust the declared schema for orientation, but verify it against the HTML before any shape-changing write.

Editing the user's saved data: set_artefact_data replaces YOUR OWN blob for an artefact (never another user's). It is WHOLE-BLOB REPLACEMENT — not a patch, nothing merged, any key you omit is deleted — and there is no undo. So: (1) read before write, always — get_artefact_data first, send back the full transformed blob, and pass the updatedAt you read as if_unmodified_since (null if you read no entry); the artefact being open in a browser is fine (an open tab only saves when the user changes something), so a refusal means the user really edited meanwhile — re-read and re-apply, don't drop the pin. (2) Use the declared schema and its example (that is what makes writing into an empty blob possible), but confirm a shape you inferred from it against get_artefact_html. (3) If authoredAgainstVersion ≠ currentPayloadVersion, or is null, the blob may be shaped for older HTML — transform it to the live shape rather than writing it back as found. (4) Keep the pre-write blob so you can revert. (5) Tell the user what will change before writing. (6) Afterwards, tell them to reload the artefact if it is open — an open tab can't overwrite your change, but only shows it after a reload. Write {} to clear.

Reading other users' data (owner only): on an artefact the user OWNS, list_artefact_data_authors lists everyone with saved data (name, email, bytes, updatedAt, authoredAgainstVersion — no data); then call get_artefact_data with author = each authorId (or email), one per person, and aggregate on your side. Each entry is verbatim and READ-ONLY — set_artefact_data never writes someone else's data, so never try to "fix" it; a fix belongs in the HTML. Shapes vary per author: check each version pin and tolerate the unexpected. Names, emails and answers are personal data the owner is reading — report them, don't republish them into an artefact.
